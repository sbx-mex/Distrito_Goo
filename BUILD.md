...
Al reemplazar `Distrito_Go_CMS_v2_actualizado.xlsx` en la raíz, `.github/workflows/actualizar-cms.yml` valida las 14 pestañas, genera WebP para imágenes pesadas, regenera los JSON, comprueba rutas y publica el resultado. Los nombres de imagen se leen desde el CMS; no es necesario editar `index.html`.

`resumen_comunicado_semana_actual.png` conserva su nombre y estrategia `network-first`. Las demás imágenes generadas incluyen una versión por contenido en los JSON para evitar caché obsoleta.
