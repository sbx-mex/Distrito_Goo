...
check('22. PWA sincronizada', /CACHE_NAME\s*=\s*`\$\{CACHE_PREFIX\}v[\w.-]+`/.test(sw) && sw.includes('staleWhileRevalidate') && sw.includes('./styles/distrito-go.css') && sw.includes('./data/desarrollo-partner.v1.json') && sw.includes('./modules/search.js') && sw.includes('./modules/operations-center.js') && sw.includes('./modules/calendar.js'), 'caché versionado, módulos y estilos consolidados');
...
check('24. Menú principal simplificado', [...html.matchAll(/data-view="(home|informativo|explore|saved)"/g)].length === 4 && !html.includes('data-view="search"') && html.includes('global-search-panel'), 'Inicio, Informativo, Explorar y Guardados; buscador dentro de Explorar');
