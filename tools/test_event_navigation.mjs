...
const ids = events.map(item => String(item.ID ?? '').trim()).filter(Boolean);
...
check('La salida contiene únicamente eventos publicados', events.every(item => item.Publicar !== false));
check('Los eventos publicados conservan nombre', events.every(item => String(item.Actividad || '').trim()));
check('Los IDs publicados son únicos', ids.length === events.length && new Set(ids).size === ids.length);
check('Las vigencias vienen del CMS y son válidas', events.every(validRange));
