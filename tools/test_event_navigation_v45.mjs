...
check('ECO opcional respeta su destino', !eco || eco.Link === 'https://app.slikpro.com/login/alsea');
check('ECO opcional conserva imagen', !eco || /eco_2026\.(?:jpe?g|webp)/i.test(eco.ImagenOriginal || eco.ImagenPath || ''));
...
check('Caché versionado', /CACHE_PREFIX.*v\d+/s.test(fs.readFileSync('sw.js', 'utf8')));
